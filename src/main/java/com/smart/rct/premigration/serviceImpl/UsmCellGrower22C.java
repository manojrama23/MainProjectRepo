...
public class UsmCellGrower22C {

	private static Logger logger = LoggerFactory.getLogger(UsmCellGrower22C.class);
	public static String supportCA;
	public static boolean AdNew = false;
	public static boolean remarks = true;

	public  static synchronized boolean createGrowTemplate_22C(ArrayList<TreeMap<String, Object>> cellDataAref,
...
*/
				...
				pw.print("\"@LTE_CBRS_ALLOWED_BANDWIDTH_INFORMATION\"\n");
				pw.print(
						"\"State\",\"Sector ID\",\"Carrier ID\",\"CBRS Bandwidth 5mhz\",\"CBRS Bandwidth 10mhz\",\"CBRS Bandwidth 15mhz\",\"CBRS Bandwidth 20mhz\"\n");
				for (int i = 0; i < addAref.size(); i++) {
					fda = addAref.get(i);
					if (fda.get("bandpcs").toString().contains("CBRS")) {
						if (fda.get("bwstr").toString().contains("5")  && !fda.get("bwstr").toString().contains("15")) {
							pw.print("\"ADD" + "\",\"" + fda.get("sectid") + "\",\"" + fda.get("carrid")
									+ "\",\"true\",\"false\",\"false\",\"false\"\n");
						} else if (fda.get("bwstr").toString().contains("10")) {
							pw.print("\"ADD" + "\",\"" + fda.get("sectid") + "\",\"" + fda.get("carrid")
									+ "\",\"false\",\"true\",\"false\",\"false\"\n");
						} else if (fda.get("bwstr").toString().contains("15")) {
							pw.print("\"ADD" + "\",\"" + fda.get("sectid") + "\",\"" + fda.get("carrid")
									+ "\",\"false\",\"false\",\"true\",\"false\"\n");
						} else if (fda.get("bwstr").toString().contains("20")) {
							pw.print("\"ADD" + "\",\"" + fda.get("sectid") + "\",\"" + fda.get("carrid")
									+ "\",\"false\",\"false\",\"false\",\"true\"\n");
						}
					}/*else {
						pw.print("\"NONE\",\"\",\"\",\"false\",\"false\",\"false\",\"false\"\n");
					}*/
				}
				...
				pw.print(
						"\"X Pole Antenna\",\"Antenna Gain dBi\",\"Cable Loss\",\"Accuracy Margin dB\",\"Sharing Flag\",\"Primary Flag\",\"Dual Personality Flag\",\"Dual Personality Primary Flag\",\"RU Copy Enable\",\"RF Port Operation Mode\"\n");
				...
				for (int i = 0; i < rrhDataAref.size(); i++) {
					...
					String rpMode="";
					if(rda.get("RFPORTOPERATIONMODE").toString().isEmpty() || rda.get("RFPORTOPERATIONMODE").toString().equals("-") ) {
						rpMode="duplex";
					}else {
						rpMode=rda.get("RFPORTOPERATIONMODE").toString();
					}
				...
				pw.print("\"@LTE_CBRS_CHANNEL_BLOCK_LISTED_INFORMATION\"\n");
				pw.print(
						"\"State\",\"Sector ID\",\"Carrier ID\",\"Block Listed Channel\"\n");
				pw.print("\"NONE\",\"\",\"\",\"\"\n");
