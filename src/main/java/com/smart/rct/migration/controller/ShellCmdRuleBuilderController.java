...
@RestController
public class ShellCmdRuleBuilderController {

	final static Logger logger = LoggerFactory.getLogger(CmdRuleBuilderController.class);

	@Autowired
	ShellCmdRuleBuilderService shellCmdRuleBuilderService;
	...
	@PostMapping(value = "/createShellCmdRuleBuilder")
	public JSONObject createShellCmdRuleBuilder(@RequestBody JSONObject createShellCmdRuleBuilderDetails) {
		...
		String subType = null;
		try {
			sessionId = createShellCmdRuleBuilderDetails.get("sessionId").toString();
			serviceToken = createShellCmdRuleBuilderDetails.get("serviceToken").toString();
			customerId = Integer.valueOf(createShellCmdRuleBuilderDetails.get("customerId").toString());
			...
			migrationType = createShellCmdRuleBuilderDetails.get("migrationType").toString();
			programId = (int) createShellCmdRuleBuilderDetails.get("programId");
			subType = createShellCmdRuleBuilderDetails.get("subType").toString();
			...
			ShellCmdRuleBuilderEntity shellCmdRuleBuilderEntity = new Gson().fromJson(
					createShellCmdRuleBuilderDetails.toJSONString((Map) createShellCmdRuleBuilderDetails.get("cmdRuleDetail")),
					ShellCmdRuleBuilderEntity.class);
			//cmdRuleBuilderEntity.setCustomerId(customerId);
			shellCmdRuleBuilderEntity.setCreationDate(new Date());
			shellCmdRuleBuilderEntity.setUseCount(0);
			shellCmdRuleBuilderEntity.setMigrationType(migrationType);
			shellCmdRuleBuilderEntity.setSubType(subType);
			User user = UserSessionPool.getInstance().getSessionUser(sessionId);
			shellCmdRuleBuilderEntity.setCreatedBy(user.getUserName());
			CustomerDetailsEntity customerDetailsEntity = shellCmdRuleBuilderService.getCustomerDetailsEntity(programId);
			shellCmdRuleBuilderEntity.setCustomerDetailsEntity(customerDetailsEntity);
			// check duplicate rule name creation
			boolean ruleNameExistence = shellCmdRuleBuilderService.duplicateFileName(shellCmdRuleBuilderEntity.getRuleName()
			...
			if (shellCmdRuleBuilderEntity != null) {
				// creating new User
				if (shellCmdRuleBuilderService.createCmdRule(shellCmdRuleBuilderEntity)) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	@PostMapping(value = "/getShellCmdRuleBuilder")
	public JSONObject getShellCmdRuleBuilder(@RequestBody JSONObject searchCmdRuleBuilderDetails) {
		...
		try {
			...
			if ("search".equals(searchStatus)) {
				...
				ShellCmdRuleBuilderModel cmdRuleBuilderModel = new Gson().fromJson(searchCmdRuleBuilderDetails.toJSONString((Map) searchCmdRuleBuilderDetails.get("searchCriteria")), ShellCmdRuleBuilderModel.class);
			...
			//	if (StringUtils.isNotEmpty(searchBy) && StringUtils.isNotEmpty(searchParameter)) {
					Map<String, Object> searchList = shellCmdRuleBuilderService.searchCmdRule(cmdRuleBuilderModel,
			...
			//	}
				...
				Map<String, Object> searchList = shellCmdRuleBuilderService.searchCmdRule(customerId, page, count,
	 ...
	 */
	@SuppressWarnings("unchecked")
	@PostMapping(value = "/updateShellCmdRuleBuilder")
	public JSONObject updateCmdRuleBuilder(@RequestBody JSONObject updateCmdRuleBuilder) {
		...
		try {
			...
			CustomerDetailsEntity customerDetailsEntity = shellCmdRuleBuilderService.getCustomerDetailsEntity(programId);
			ShellCmdRuleBuilderEntity cmdRuleEntity = new Gson().fromJson(
...
//			}
			if (cmdRuleEntity != null) {
				...
				if (shellCmdRuleBuilderService.updateCmdRuleBuilder(cmdRuleEntity)) {
					mapObject.put("status", Constants.SUCCESS);
					commonUtil.saveAudit(Constants.EVENT_RULES, Constants.EVENT_RULES_COMMAND_RULE_BUILDER,
	 ...
	 */
	@SuppressWarnings("unchecked")
	@PostMapping(value = "/deleteShellCmdRuleBuilder")
	public JSONObject deleteCmdRuleBuilder(@RequestBody JSONObject deleteCmdRuleBuilder) {
		...
		try {
			...
			if (StringUtils.isNotEmpty(id)) {
				if (shellCmdRuleBuilderService.deleteCmdRule(Integer.valueOf(id))) {
