...
public class PreMigrationToOV {
	...
	public JSONObject getTrakerIdList(JSONObject ovUpdateJson, GenerateInfoAuditEntity runTestEntityMap) {
		...
		try {
			...
			System.out.println(FuzeProjectID);
				...
				/*if (programName.contains("CBAND")) {
					...
					URL = Constants.OV_TRACKERID_URL + "page=1&per_page=100000&" +"P_FUZE_PROJECT_ID_FZ="+FuzeProjectID+"&P_5G_DSS_SCOPE=1";
	...
	@SuppressWarnings("unchecked")
	public JSONObject getOvEnvUploadDetails(JSONObject ovUpdateJson,GenerateInfoAuditEntity generateInfoAuditEntity1) {
		...
		try {
			...
			String date2=DateUtil.dateToString(new Date(), "yyyy-MM-dd HH:mm:ss");
				...
				filePath.append(ovUpdateJson.get("filePath")).append(ovUpdateJson.get("FileName"));
			...
			logger.error("request Entity: "+ requestEntity);
				...
				if (200 == statusCode) {
					...
					premigrationOvUpadteEntity.setCurrentResult("["+date2+"]"+"-"+"Successfully File :"+FileName +" Uploaded To OV");

					runTestRepository.updatePreMigrationOv(premigrationOvUpadteEntity);
					generateInfoAuditEntity1.setOvUpdateStatus("SUCCESS");
					objGenerateCsvService.updateGeneratedFileDetails(generateInfoAuditEntity1);
					...
					premigrationOvUpadteEntity.setFileName(ovUpdateJson.get("FileName").toString());
					premigrationOvUpadteEntity.setFilePath(ovUpdateJson.get("filePath").toString());
					premigrationOvUpadteEntity.setCurrentResult("["+date2+"]"+"-"+"Failed to  updated the " +FileName);
					runTestRepository.updatePreMigrationOv(premigrationOvUpadteEntity);
