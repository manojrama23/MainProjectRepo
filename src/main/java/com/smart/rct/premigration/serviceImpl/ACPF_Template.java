...
public class ACPF_Template {
	...
	public String generateAcpfTemplate(ArrayList<String> ciqDataTemp, String ver, String outputdir, String gnodebID, String relversion)
	{
		ciqData = ciqDataTemp;
		version = ver;
	...
	private void populate_Necessary_Data()
	{
		...
		String tags[] = {"market", "neidacpf", "network", "neidacpfname", "gnbcuname", "gndbid", "gndbauid" };
		String headers[] = {"MARKET", "NE_ID_ACPF", "NETWORK", "NE_ID_ACPF_NAME", "GNB_CU_NAME", "GNODEB_ID", "GNODEB_AUID" };
		...
		{			
			for(int k=0;k<scrubData.size();k++)
			{
				...
				if(gndbAuId.get(gnbidx).equals(lineA[find_necessaryA_idx("gndbauid")]))
				{
					...
					index = find_necessaryA_idx("neidacpf");
					if(index != -1)
					{
						cellDataH.put("neidacpf", lineA[index]);
					}
					...
					index = find_necessaryA_idx("neidacpfname");
					if(index != -1)
					{
						cellDataH.put("neidacpfname", lineA[index]);
					}
	...
	{
		String growfile1 = growfolder + "/ACPF_" + version + "_" + cellDataRef.get(0).get("neidacpf") + ".csv";
		for(int i=0;i<cellDataRef.size();i++)
		{
			...
			{		
				...
				pw1.print("\"" + cellDataH.get("neidacpf") + "\",\"TBD\",\"gnb_cu_up\",\""+ ver + "\",\"" + relver + "\",\"" + cellDataH.get("network") + "\",\"" + cellDataH.get("neidacpfname") + "\",\"0\"\n");
				...
				pw1.print("\"NE ID\",\"gNB ID\",\"gNB ID Length\",\"Local IP Address\",\"gNB CU Name\"\n");
				pw1.print("\"" + cellDataH.get("neidacpf") + "\",\"" + cellDataH.get("gndbid") + "\",\"22\",\"TBD\",\"" + cellDataH.get("gnbcuname")+ "\"\n");
				
				pw1.print("\"@ENDPOINT_X2_INFORMATION\"\n");
				pw1.print("\"NE ID\",\"gNB ID\",\"Endpoint X2 IP address\"\n");
				pw1.print("\"" + cellDataH.get("neidacpf") + "\",\"" + cellDataH.get("gndbid") +"\",\"TBD\"\n");
				
				pw1.print("\"@GEO_REDUNDANCY_INFORMATION\"\n");
				pw1.print("\"NE ID\",\"Data Center ID\",\"HAC Flag\",\"Primary HAC IP Address\",\"Secondary HAC IP Address\",\"Primary VDB IP Address\",\"Secondary VDB IP Address\",\"HAC Reconnection Timeout\",\"HAC Registration Timeout\"\n");
				pw1.print("\"\",\"\",\"\",\"\",\"\",\"\",\"\",\"\",\"\"\n");
				
				pw1.print("\"@RESERVED_GNB_IP_INFORMATION\"\n");
				pw1.print("\"NE ID\",\"Reserved gNB IP Address\"\n");
				pw1.print("\"\",\"\"");
		...
		return "ACPF_"+ version +"_" + cellDataRef.get(0).get("neidacpf") + ".csv";
	}
