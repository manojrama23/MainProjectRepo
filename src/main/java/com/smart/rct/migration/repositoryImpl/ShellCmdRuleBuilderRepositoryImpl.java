...
@Repository
@Transactional
public class ShellCmdRuleBuilderRepositoryImpl implements ShellCmdRuleBuilderRepository {
	...
	public Map<String, Object> loadCmdRuleBuilderSearchDetails(int customerId, int page, int count,
		...
		List<ShellCmdRuleBuilderModel> cmdRuleBuilderList = new ArrayList<>();
		...
		try {
			...
			Criteria criteria = entityManager.unwrap(Session.class).createCriteria(ShellCmdRuleBuilderEntity.class);
			...
			List<ShellCmdRuleBuilderEntity> cmdRuleBuilderEntityList = criteria
			...
			if (cmdRuleBuilderEntityList != null && cmdRuleBuilderEntityList.size() > 0)
				for (ShellCmdRuleBuilderEntity objCmdRuleBuilderEntity : cmdRuleBuilderEntityList) {

					ShellCmdRuleBuilderModel objCmdRuleBuilderModel = new ShellCmdRuleBuilderModel();
					...
					objCmdRuleBuilderModel.setResultType(objCmdRuleBuilderEntity.getResultType());
					cmdRuleBuilderList.add(objCmdRuleBuilderModel);

				}

			Criteria criteriaCount = entityManager.unwrap(Session.class).createCriteria(ShellCmdRuleBuilderEntity.class);
	 ...
	 */
	public ShellCmdRuleBuilderEntity getById(int id) {
	...
	public boolean findByRuleName(String ruleName, String migrationType, int programId,
		...
		ShellCmdRuleBuilderEntity dupCmdEntity = null;
		boolean status = false;
		try {
			CustomerDetailsEntity customerDetailsEntity = entityManager.find(CustomerDetailsEntity.class, programId);
			CriteriaBuilder builder = entityManager.getCriteriaBuilder();
			CriteriaQuery<ShellCmdRuleBuilderEntity> query = builder.createQuery(ShellCmdRuleBuilderEntity.class);
			...
			TypedQuery<ShellCmdRuleBuilderEntity> queryResult = entityManager.createQuery(query);
			dupCmdEntity = (ShellCmdRuleBuilderEntity) queryResult.getResultList().stream().findFirst().orElse(null);
	...
	public String findCommand(String ruleName,String migrationType,String subType,int programId) {
		...
		try {
			...
			ShellCmdRuleBuilderEntity cmdRuleBuilderEntity = typedQuery.getSingleResult();
	...
	public Map<String, Object> loadCmdRuleBuilderSearchDetails(ShellCmdRuleBuilderModel cmdRuleBuilderModel, int customerId,
		...
		try {
			...
			criteria.setMaxResults(count);
			criteria.addOrder(Order.desc("creationDate"));
			List<ShellCmdRuleBuilderEntity> cmdRuleBuilderEntityList = 
